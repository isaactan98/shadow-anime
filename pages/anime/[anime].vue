        ...
        <UContainer class="mt-5">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-5">
                <div class=" col-span-3">
                    <h1 class=" text-white text-xl md:text-4xl">{{ animeMeta?.title.english ?? anime.title }}</h1>
                    <h5 class="text-zinc-400 md:text-lg my-2">{{ anime.otherName }}</h5>
                    <div class="flex gap-5 items-center mt-3">
                        <p class="text-zinc-300 capitalize">{{ anime.subOrDub }}</p>
                        <UBadge color="gray">{{ animeMeta?.status ?? anime.status }}</UBadge>
                    </div>
                    <div class="flex my-3 items-center">
                        <UButton label="Add to Watchlist" color="gray" />
                    </div>
                    <div class="mt-3 h-24 overflow-y-auto">
                        <p v-html="animeMeta?.description ?? anime.description" class="text-white text-justify text-sm"></p>
                    </div>
                    ...
                    <div class="mt-5 w-full" v-if="animeMeta != null && animeMeta.episodes.length == anime.episodes.length">
                        ...
                        <div class="mt-3">
                            <NuxtLink v-for="(e, i) in anime.episodes" :key="i"
                                ...
                                <img :src="animeMeta.episodes[i].image" alt="" class=" w-1/4 h-28 rounded-lg object-cover">
                                <div class="w-3/4">
