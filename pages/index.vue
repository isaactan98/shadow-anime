<template>
    <div class="w-full mx-auto">
        <UCarousel v-if="slides.length > 0" v-slot="{ item, index }" :items="slides" arrows indicators>
            <div class="w-screen object-cover flex justify-end items-center overflow-hidden relative h-[55vh] md:h-[80vh]">
                <img :src="item.image" class="h-full w-5/6 md:h-[80vh] object-cover"
                    style="mask-image: linear-gradient(270deg,transparent 0,rgb(36,36,40) 10%,rgb(36,36,40) 50%,transparent);">
                <div
                    class="absolute top-3/4 md:top-2/3 left-10 w-full font-semibold drop-shadow-sm text-md md:text-5xl text-white">
                    <UContainer>
                        <h1 class="mb-3">
...
export default {
    ...
    async mounted() {
        this.recentRelease = await anilistRecentReleases()
        console.log('recentRelease', this.recentRelease)
        this.topAiring = await anilistAiringSchedule()
